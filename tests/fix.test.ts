...
import { selectTractable, applyFix, revertTo } from '../scripts/lib/fix.ts';
import { REASONS } from '../scripts/lib/findings-schema.ts';
import type { Finding, Severity } from '../scripts/lib/findings-schema.ts';
import { makeTempRepo, write, initGit, git, cleanup } from './helpers/fixtures.ts';

const toClean: string[] = [];
after(() => toClean.forEach(cleanup));
const repo = (): string => { const d = makeTempRepo('wont-scale-fix-'); toClean.push(d); return d; };
const F = (reason: number, severity: Severity): Finding => ({ reason, slug: REASONS[reason].slug, status: 'finding', severity, evidence: ['x'] });
...
test('a clean in-target fix applies and is contained', async () => {
  ...
  initGit(r);
  const drive = async (_p: string, root: string) => writeFileSync(join(root, 'src', 'webhook.js'), 'export function h() { if (seen(id)) return; charge(); }\n');
...
test('a fix that writes .git/hooks is caught and reverted loudly', async () => {
  ...
  write(r, 'src/a.js', 'x');
  initGit(r);
  const drive = async (_p: string, root: string) => {
...
test('a dirty tree isolates the reader work; revert preserves it (no data loss)', async () => {
  ...
  const drive = async (_p: string, root: string) => writeFileSync(join(root, 'src', 'b.js'), 'the fix');
  const res = await applyFix({ root: r, finding: F(6, 'high'), prompt: 'fix', drive });
  assert.equal(res.applied, true);
...
test('a fix writing an unexpected out-of-diff path trips containment', async () => {
  ...
  const drive2 = async (_p: string, root: string) => {
    writeFileSync(join(root, 'src', 'a.js'), 'fixed');
