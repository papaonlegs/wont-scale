...
import { runMechanical, reconcile } from '../scripts/lib/mechanical.mjs';
import { validateFinding } from '../scripts/lib/findings-schema.mjs';
import { plantedDefectRepo, cleanRepo, cleanup } from './helpers/fixtures.mjs';
...
test('AE9 reconcile forces a clean drive result to not-verified when a mechanical check fired', () => {
  ...
  const drive = [{ reason: 3, slug: 'authentication', status: 'clean', severity: 'critical', evidence: ['all good'] }];
...
test('reconcile leaves an honest clean result alone', () => {
  const root = cleanRepo(); toClean.push(root);
  const mechanical = runMechanical(root);
  const drive = [{ reason: 8, slug: 'observability', status: 'clean', severity: 'high', evidence: ['sentry configured'] }];
  const reconciled = reconcile(drive, mechanical);
