 ...
 *
 *   node dist/assemble.js --all              regenerate committed aggregates
 *   node dist/assemble.js --guardrails [--tool cursor|copilot|windsurf|agents]
 *   node dist/assemble.js --audit-prompt 4   drive prompt for one reason
 *   node dist/assemble.js --fix-prompt '<finding-json>'
 *   node dist/assemble.js --reason-index     the wizard's REASONS data as JSON
 *   node dist/assemble.js --digest           the taxonomy digest (KTD7)
 *   node dist/assemble.js --checklist        the regenerated CHECKLIST tier line
 */
...
function writeIfChanged(path: string, content: string, rel: string, changed: string[]): void {
...
function main(argv: string[]): void {
  ...
  switch (flag) {
    ...
    case '--fix-prompt': process.stdout.write(fixPrompt(JSON.parse(rest[0]) as Finding)); return;
    ...
    default:
      console.error('usage: assemble.js --all | --guardrails [--tool <name>] | --audit-prompt <n> | --fix-prompt <json> | --reason-index | --digest | --checklist');
      process.exit(2);
  }
}
